...
class Program
{
    static void Main(string[] args)
    {
        ...
        {
            targetFile = "C:\\Prosoft\\EasySave\\Destination" + "\\" + Path.GetFileNameWithoutExtension(fileName);       //  [1] = target_directory for decrypting
            timeElapsed = DecryptFile(sourceFile, targetFile, encryptionKey, stopwatch);
        }
        else
        {
            targetFile = "C:\\Prosoft\\EasySave\\Destination" + "\\" + fileName + ".cry";       //  [1] = target_directory for encrypting
    ...
    static TimeSpan EncryptFile(string inputFile, string outputFile, string key, Stopwatch stopwatch)
    {
    ...
    static TimeSpan DecryptFile(string inputFile, string outputFile, string key, Stopwatch stopwatch)
    {
        using (FileStream inputStream = new FileStream(inputFile, FileMode.Open, FileAccess.Read))
        using (FileStream outputStream = new FileStream(outputFile, FileMode.Create, FileAccess.Write))
        {
